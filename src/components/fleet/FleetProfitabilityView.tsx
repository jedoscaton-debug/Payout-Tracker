...
export function FleetProfitabilityView({ routeTracker, settings }: FleetProfitabilityViewProps) {
  ...
  const vanStats = useMemo(() => {
    ...
    return Object.values(groups).map(g => {
      ...
      const redLimit = settings?.fleetRedThreshold ?? 100;
      const yellowLimit = settings?.fleetYellowThreshold ?? 300;
  ...
  const handleExport = async () => {
    const element = document.getElementById("fleet-report-board");
    if (!element) return;
    
    setIsExporting(true);
    toast({ title: "Preparing Report", description: "Generating high-fidelity PDF board..." });
    
    try {
      // @ts-ignore
      const html2pdf = (await import('html2pdf.js')).default;
      const fileName = `Fleet_Profitability_Report_${startDate}_to_${endDate}.pdf`;
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <style jsx global>{`
        @media print {
          ...
          #fleet-report-board { padding: 10mm !important; background: white !important; }
        }
      `}</style>
      ...
      <div id="fleet-report-board" className="space-y-8">
        <Collapsible open={isLogicOpen} onOpenChange={setIsLogicOpen} className="no-print bg-slate-50 rounded-2xl border border-slate-200 overflow-hidden">
