...
import { PayrollItem, PayrollRun, ComputedTotals } from "@/app/lib/types";
...
export function PaystubPreview({ item, run }: PaystubPreviewProps) {
  ...
  return (
    <div className="grid h-full gap-0 overflow-hidden lg:grid-cols-[1fr_300px]">
      <ScrollArea className="h-full bg-slate-50 p-4 sm:p-8">
        <div className="mx-auto max-w-2xl rounded-[2rem] border border-slate-200 bg-white p-8 shadow-sm print:shadow-none print:border-none">
          ...
          <div className="mt-10 space-y-8">
            <section>
              <h3 className="mb-4 text-xs font-bold uppercase tracking-widest text-accent">Earnings Breakdown</h3>
                ...
                <table className="w-full text-sm">
                  ...
                  <tfoot>
                    <tr className="bg-accent/5 font-bold">
                      <td className="px-4 py-4 text-accent uppercase tracking-wide">Total Gross Earnings</td>
                      <td className="px-4 py-4 text-right text-accent">{currency(totals.grossPay)}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
            ...
            <section>
              <h3 className="mb-4 text-xs font-bold uppercase tracking-widest text-rose-600">Deductions</h3>
              <div className="rounded-2xl border border-slate-100 bg-slate-50/50 overflow-hidden">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-slate-100/50">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {item.deductionsLines.map((line) => (
                      <tr key={line.id}>
                        ...
                        <td className="px-4 py-3 text-right font-medium text-rose-600">({currency(line.amount)})</td>
                      </tr>
                    ))}
                  </tbody>
                  <tfoot>
                    <tr className="bg-rose-50 font-bold">
                      <td className="px-4 py-4 text-rose-700 uppercase tracking-wide">Total Deductions</td>
                      <td className="px-4 py-4 text-right text-rose-700">{currency(totals.totalDeductions)}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
          ...
          <div className="mt-12 flex items-center justify-between rounded-3xl bg-slate-900 p-8 text-white shadow-xl shadow-slate-200">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-slate-400">Net Payment Amount</p>
              <p className="text-4xl font-black tracking-tighter">{currency(totals.netPay)}</p>
            </div>
            <div className="text-right">
              <p className="text-sm font-medium text-slate-400">Direct Deposit</p>
              <p className="text-xs text-slate-500 uppercase tracking-wider mt-1">Confirmed</p>
            </div>
          </div>

          {item.notes && (
            <div className="mt-8 p-6 rounded-2xl border border-slate-100 bg-slate-50/30 italic text-slate-600 text-sm">
              <span className="font-bold uppercase tracking-tighter text-[10px] block mb-1 not-italic text-slate-400">Special Instructions</span>
              {item.notes}
            </div>
          )}
        </div>
      ...
      <div className="hidden flex-col gap-4 border-l border-slate-200 bg-white p-6 lg:flex">
        <h3 className="text-sm font-bold uppercase tracking-widest text-slate-400">Quick Actions</h3>
        <Button className="w-full rounded-2xl h-12 bg-primary font-semibold" onClick={handlePrint}>
          <Printer className="mr-2 h-4 w-4" /> Print Paystub
        </Button>
        <Button variant="outline" className="w-full rounded-2xl h-12 border-slate-200 text-slate-600">
