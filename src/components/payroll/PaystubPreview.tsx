...
import { FileDown, Printer, Share2, Clipboard } from "lucide-react";
...
export function PaystubPreview({ item, run }: PaystubPreviewProps) {
  ...
  const handleShare = async () => {
    ...
    if (typeof navigator !== 'undefined' && navigator.share) {
      try {
        ...
        // Fallback to clipboard if share is cancelled or restricted
        copyToClipboard();
      }
    } else {
      copyToClipboard();
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full w-full overflow-hidden bg-white">
      {/* Main Content Area */}
      <div className="flex-1 h-full overflow-hidden bg-slate-50 border-r border-slate-200">
        <ScrollArea className="h-full p-4 sm:p-8 print:bg-white print:p-0">
      ...
      {/* Sidebar Actions */}
      <div className="w-full lg:w-[300px] flex flex-col gap-4 bg-white p-6 print:hidden shadow-lg lg:shadow-none">
        ...
        <div className="mt-auto rounded-2xl bg-slate-50 p-4 border border-slate-100">
          ...
          <pre className="text-[10px] bg-slate-900 text-slate-300 p-3 rounded-lg overflow-auto max-h-[150px] font-code">
